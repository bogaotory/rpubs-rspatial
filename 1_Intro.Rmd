...
Note: default working directory is `/home/your_user_name`. If your `table_10.csv` is at `~/Downloads/table_10.csv`, without creating a new directory as in 1-3:
```
dfm <- read.csv("./Downloads/table_10.csv")
```
5. _OPTIONAL:_ Try to use the `read.table()` function. What is different?

See [https://stackoverflow.com/questions/12828438/read-csv-vs-read-table] for description of the differences.
```
dfm <- read.table("./Downloads/table_10.csv", header= TRUE, sep = ",")
```
...
```
dfm[dfm$division == "New England",] # The comma is important
```
...
```
dfm$pctIL <- dfm$totalIL/dfm$totalpop
```
10. _FOR FUN:_
```{r eval=FALSE}
...
```{r}
melt(dfm[names(dfm)[1:4]], id.vars = c("state", "division"))
```
